...
from saleor.dashboard.shipping.forms import (
    PriceShippingMethodForm,
    ShippingZoneForm,
    WeightShippingMethodForm,
...
def test_shipping_zone_form():
    ...
    assert "DE" not in [code for code, name in form.fields["countries"].choices]
...
def test_create_duplicated_default_shipping_zone_form(shipping_zone):
    ShippingZone.objects.create(name="Zone", default=True)
    form = ShippingZoneForm(
...
def test_add_default_shipping_zone_form():
    form = ShippingZoneForm(data={"name": "Zone", "countries": ["PL"], "default": True})
    assert form.is_valid()
    zone = form.save()
    assert zone.default
...
def test_shipping_zone_edit(admin_client, shipping_zone):
    assert ShippingZone.objects.count() == 1
    url = reverse("dashboard:shipping-zone-update", kwargs={"pk": shipping_zone.pk})
...
def test_shipping_zone_details(admin_client, shipping_zone):
    assert ShippingZone.objects.count() == 1
    url = reverse("dashboard:shipping-zone-details", kwargs={"pk": shipping_zone.pk})
...
def test_shipping_zone_delete(admin_client, shipping_zone):
    assert ShippingZone.objects.count() == 1
    url = reverse("dashboard:shipping-zone-delete", kwargs={"pk": shipping_zone.pk})
    response = admin_client.post(url, follow=True)
    assert response.status_code == 200
    assert ShippingZone.objects.count() == 0
...
def test_shipping_method_add(admin_client, shipping_zone):
    ...
    url = reverse(
        ...
        kwargs={"shipping_zone_pk": shipping_zone.pk, "type": "price"},
    )
    data = {
        "name": "DHL",
        "price_0": "50",
