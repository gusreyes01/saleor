...
def test_view_product_select_type_by_ajax(admin_client, product_type):
    ...
    response = admin_client.post(url, data, HTTP_X_REQUESTED_WITH="XMLHttpRequest")
...
def test_view_product_not_deleted_before_confirmation(db, admin_client, product):
    url = reverse("dashboard:product-delete", kwargs={"pk": product.pk})
...
def test_view_ajax_products_list(admin_client, product):
    ...
    assert response.status_code == 200
    resp_decoded = json.loads(response.content.decode("utf-8"))
    assert resp_decoded.get("results") == [{"id": product.id, "text": str(product)}]
...
def test_view_product_type_create(admin_client, color_attribute, size_attribute):
    ...
    assert response.status_code == 302
    assert get_redirect_location(response) == reverse("dashboard:product-type-list")
...
def test_view_product_type_delete(db, admin_client, product):
    ...
    url = reverse("dashboard:product-type-delete", kwargs={"pk": product_type.pk})
...
def test_view_product_type_not_deleted_before_confirmation(admin_client, product):
    product_type = product.product_type
    url = reverse("dashboard:product-type-delete", kwargs={"pk": product_type.pk})
...
def test_view_product_variant_not_deleted_before_confirmation(admin_client, product):
    variant = product.variants.get()
    url = reverse(
        "dashboard:variant-delete",
        kwargs={"product_pk": product.pk, "variant_pk": variant.pk},
...
def test_view_ajax_available_variants_list(admin_client, product, category, settings):
...
def test_view_product_images(admin_client, product_with_image):
    product_image = product_with_image.images.get()
    url = reverse(
        "dashboard:product-image-list", kwargs={"product_pk": product_with_image.pk}
    )
...
def test_view_product_image_create(monkeypatch, admin_client, product_with_image):
    ...
    url = reverse(
        "dashboard:product-image-add", kwargs={"product_pk": product_with_image.pk}
    )
...
def test_view_ajax_upload_image(monkeypatch, admin_client, product_with_image):
    ...
    url = reverse("dashboard:product-images-upload", kwargs={"product_pk": product.pk})
    image, image_name = create_image()
    data = {"image_0": image, "alt": ["description"]}
...
def test_view_attribute_details(admin_client, color_attribute):
    url = reverse("dashboard:attribute-details", kwargs={"pk": color_attribute.pk})
...
def test_view_attribute_details_no_choices(admin_client):
    ...
    url = reverse("dashboard:attribute-details", kwargs={"pk": attribute.pk})
...
def test_view_attribute_edit(color_attribute, admin_client):
    url = reverse("dashboard:attribute-update", kwargs={"pk": color_attribute.pk})
...
def test_view_attribute_delete(admin_client, color_attribute):
    url = reverse("dashboard:attribute-delete", kwargs={"pk": color_attribute.pk})
...
def test_view_attribute_value_create(color_attribute, admin_client):
    ...
    url = reverse(
        "dashboard:attribute-value-add", kwargs={"attribute_pk": color_attribute.pk}
    )
...
def test_view_attribute_value_create_invalid(color_attribute, admin_client):
    values = AttributeValue.objects.filter(attribute=color_attribute.pk)
    assert values.count() == 2
    url = reverse(
        "dashboard:attribute-value-add", kwargs={"attribute_pk": color_attribute.pk}
...
def test_view_attribute_value_edit(color_attribute, admin_client):
    ...
    assert response.status_code == 200
    values = AttributeValue.objects.filter(attribute=color_attribute.pk, name="Pink")
    assert len(values) == 1
...
def test_view_attribute_value_delete(color_attribute, admin_client):
    ...
    url = reverse(
        ...
        kwargs={"attribute_pk": color_attribute.pk, "value_pk": deleted_value.pk},
    )
...
def test_view_ajax_reorder_attribute_values(admin_client, color_attribute):
...
def test_view_ajax_reorder_attribute_values_invalid(admin_client, color_attribute):
    order_before = [val.pk for val in color_attribute.values.all()]
    ordered_values = list(reversed(order_before))
